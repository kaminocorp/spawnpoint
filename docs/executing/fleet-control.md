...
6. **Frontend deploy modal** — gains a **"Deployment"** step (region, size preset, **volume size**, replica count, restart policy, lifecycle mode) between the existing "Model" step and submit. Region dropdown is populated from `ListDeploymentRegions` (cached client-side per session). A **"Review"** screen shows the resolved Fly machine config (region, size preset, replicas, volume size) before submit. *(Cost estimation cut per Q5 / decision 15.)*
...
3. **Reverse preset lookup → click-to-select preset chips, no freeform default.** The deploy modal's "Size" surface is a row of preset chips (`shared-cpu-1x · 512MB`, `shared-cpu-2x · 1GB`, etc.) — one tap fills both fields. A trailing **"Custom…"** chip reveals two number inputs (CPUs, MemoryMB) for power users. Default selection = the smallest preset (`shared-cpu-1x · 512MB`, mirrors the control plane). The fleet row "Size" column displays the resolved preset name on exact match; off-preset tuples render as `Custom · 2x · 1.5GB`. No nearest-fit. *(Affects Phase 6 deploy modal AND Phase 7 fleet row "Size" column.)*
...
5. **Pricing entirely cut from M5.** No JSON, no refresh cadence, no per-row cost column. Decision 15 in §2 is **superseded** — strike `frontend/src/lib/fly-pricing.ts` from Phase 6 / Phase 10; the "Review" screen renders the resolved Fly machine config without an estimated-cost line. The §1.6 deploy modal description's "+ estimated monthly cost (compute + storage)" is removed. Revisit only if operators ask for it post-M5.
6. **Skeleton/loading for the deployment inspector first `dry_run` round-trip = reuse `<DeployModal>`'s submitting pattern.** Same skeleton register; consistent operator UX.
