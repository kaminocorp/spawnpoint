...
6. **Frontend spawn wizard** — `frontend/src/components/spawn/wizard.tsx` Step 4 (Deployment) is wired with the M5 fields (region, size preset, **volume size**, replica count, restart policy, lifecycle mode); the Phase-5 placeholder rows for `REGION / SIZE / VOLUME / RESTART` (currently rendered as `[ COMING WITH FLEET CONTROL ]` real-interface stubs per blueprint §11.4) become live inputs, and the existing `lifecycle` + `replicas` fields move from "collected client-side, dropped at the request boundary" to "carried on the wire as `DeployConfig`." Region dropdown is populated from `ListDeploymentRegions` (cached client-side per session). Step 5 (Review) — already in place — gains the resolved Fly machine config rows (region, size preset, replicas, volume size) before deploy. *(Cost estimation cut per Q5 / decision 15.)* (Supersedes the earlier "deploy modal" framing; the M4 modal was deleted in 0.9.2.)
...
- **Pricing display in the FE.** Cut entirely from M5 per Q5 resolution. No `frontend/src/lib/fly-pricing.ts`, no per-row cost column, no estimated-monthly-cost line in the Review step. Operators consult https://fly.io/docs/about/pricing/ directly. Revisit if operators ask for it post-M5; will need its own milestone since pricing introduces a refresh-cadence question that doesn't have a good answer today.
...
| 10 | **Capability gating uses `flaps.GetPlacements` as a pre-flight, not as a UI source** | The spawn wizard's Step 4 region dropdown shows all non-deprecated regions. On submit, the BE calls `flaps.GetPlacements({Guest, Region, Count, Org})` to validate the (size + region + count + org) tuple before `flaps.Launch`. On placement failure, surface `ErrPlacementUnavailable` with a structured detail (which axis failed) | `GetPlacements` is the closest thing Fly has to capability discovery (per token + org + size). Gating the dropdown on it would mean N region-checks per page load. Pre-flighting on submit is the right balance: zero render overhead, structured error before we orphan a Fly app. The "which axis failed" detail comes from parsing `flaps.GetPlacements`'s `RegionPlacement` array (regions with `Count: 0` are the ones that can't fit) |
...
| 15 | **Pricing display: CUT from M5** *(superseded 2026-04-26 by Q5 resolution)* | No `frontend/src/lib/fly-pricing.ts`, no rate card, no per-row cost column, no estimated-monthly-cost line in the Review step. The spawn wizard's Step 5 (Review) shows the resolved Fly machine config (region, size preset, replica count, volume size) without any cost figure | Refresh cadence has no good answer (Fly publishes no programmatic pricing endpoint), and a stale rate card is worse than no rate card — it implies authority Corellia can't deliver on. Operators consult https://fly.io/docs/about/pricing/ directly. Revisit only if operators ask for it post-M5; would need its own milestone |
| 16 | **`MachineGuest.SetSize` for preset resolution** | The spawn wizard's Step 4 "size" surface shows preset names (`shared-cpu-1x` etc.); the BE resolves to `(CPUKind, CPUs, MemoryMB)` via `MachineGuest.SetSize(name)` (fly-go helper at `machine_types.go`); the resolved tuple is what's persisted to `agent_instances`. The preset *name* is not stored | Storing the resolved tuple keeps the column types primitive (decision 5) and decouples Corellia from fly-go's preset naming if Fly ever renames. The cost: editing back to a preset later requires a reverse lookup, which is a small helper (~10 LOC) |
...
| 21 | **`SpawnN`'s `count` and `desired_replicas` are different concepts** | `SpawnN(count=5)` creates 5 *separate agents* (5 Fly apps, 5 DB rows), each with `desired_replicas=1` by default. `desired_replicas=5` on a single agent means 1 Fly app, 5 machines, 1 DB row. The spawn wizard's Step 4 (Deployment) carries `replicas` per agent. **N.B.** the wizard at 0.9.x is single-spawn only (Phase 5 decision 11); the `spawnNAgents` RPC stays on the wire but is unreached from the wizard UI. Restoring "Deploy N" requires either adding a count/prefix surface (a new pre-step or a Step 4 row) or moving the multi-spawn flow to a separate "fleet bulk spawn" surface — that decision is M5-adjacent but not strictly M5 scope; track separately if M5 wants to ship it | Conflating these would lose the semantic difference between "five sales agents, one per region" and "one sales agent, five replicas for capacity." Keeping them separate keeps the demo-moment intact ("Deploy 5") once the multi-spawn UI is restored, while adding the capacity dimension now |
...
| 27 | **The pre-flight `CheckDeploymentPlacement` RPC is exposed for use by the FE during preview, not just submit** | The spawn wizard's Step 5 (Review) calls `CheckDeploymentPlacement(region, size, replicas)` and renders a green "✓ Placement available" or red "✗ Region cannot fit this size" before the user clicks Deploy. Single endpoint, two callers (FE preview, BE submit pre-flight) | Pre-flight UX prevents the embarrassing "you fill out the form, click submit, get a Fly error" path. The endpoint is the same code path both times; FE just calls it earlier |
...
### Phase 6 — FE: spawn wizard Step 4 (Deployment) wired to `DeployConfig`

**Goal:** Spawn flow surfaces the new knobs. The Step-4 stub rows that 0.9.1 shipped as `[ COMING WITH FLEET CONTROL ]` (per blueprint §11.4) become live inputs; `lifecycle` + `replicas` (already collected client-side) start riding the wire as part of `DeployConfig`.

- Step 4 (Deployment) in `frontend/src/components/spawn/wizard.tsx` becomes the home for these fields (replaces the deleted `frontend/src/components/agents/deploy-modal.tsx` referenced in earlier drafts):
  - **Region** dropdown — populated from `ListDeploymentRegions` on wizard mount; cached in component state for the wizard's lifetime (in-memory, decision 8); default to org's primary (TBD: today, hardcoded `iad`; org-level default is post-M5).
  - **Size** — preset chips (`shared-cpu-1x` through `performance-8x`) per Q3 / decision 3; GPU presets hidden. Each chip shows `{cpus} vCPU / {memMB}MB`.
  ...
  - **Replicas per agent** — number input, 1–10 (the existing field's cap matches decision 20). Default 1. Tooltip: "How many machines to run for this agent. Use >1 for capacity. **Note: each replica gets its own volume — replicas don't share state.**"
  - **Restart policy** — radio: `on-failure (default)` / `always` / `no`. When `on-failure`: show "Max retries" number input (default 3).
  - **Lifecycle** — the existing `always-on (default)` / `manual` Select stays. Tooltip on `manual`: "Agent only runs when you start it from the fleet page." Disabled options for `idle-on-demand` and `suspended` with "Coming when secure agent endpoints ship" tooltip.
- Step 5 (Review) — already in place — gains rows for the resolved Fly machine config, calls `CheckDeploymentPlacement` for the green/red affordance, shows estimated monthly cost from `frontend/src/lib/fly-pricing.ts` **(compute + storage; storage rate is `$0.15/GB/month` per Fly's pricing page, hardcoded alongside compute rates)**. Deploy button disabled if placement is red. *(Pricing: cut per Q5 / decision 15 — this row stays superseded.)*
- "Deploy 5" path: **the wizard at 0.9.x has no multi-spawn UI** (Phase 5 decision 11 dropped it; `spawnNAgents` RPC remains on the wire). M5 scope decision: either restore the count/prefix surface as a pre-step here, or punt multi-spawn to a separate "fleet bulk spawn" milestone. If kept in M5 Phase 6, the `DeployConfig` shape applies to all N agents identically; if punted, this bullet drops out.
...
## 7. Open questions — RESOLVED (2026-04-26 operator interview)

All ten resolved before Phase 1 kicks off. Recorded here as the binding answer; the corresponding decisions in §2 supersede in case of conflict (none currently).

1. **Org-level region default → DEFERRED to M6. Default = `sin`; user-toggleable.** Spawn wizard Step 4's region dropdown defaults to `sin` (matches the control plane's primary region in `backend/fly.toml`). User can pick any non-deprecated region from the cached `ListDeploymentRegions` result. Per-org default punted to M6. *(Affects Phase 6 default value.)*
2. **`flaps.WaitForState` timeout = 60s.** Less than M4's 90s `pollHealth` because Update is a Fly state flip (no harness port-bind). Long enough to cover an unlucky cold path (typical machine state transitions <10s); short enough that the FE submitting spinner doesn't overstay. *(Affects Phase 3.)*
3. **Reverse preset lookup → click-to-select preset chips, no freeform default.** The spawn wizard Step 4's "Size" surface is a row of preset chips (`shared-cpu-1x · 512MB`, `shared-cpu-2x · 1GB`, etc.) — one tap fills both fields. A trailing **"Custom…"** chip reveals two number inputs (CPUs, MemoryMB) for power users. Default selection = the smallest preset (`shared-cpu-1x · 512MB`, mirrors the control plane). The fleet row "Size" column displays the resolved preset name on exact match; off-preset tuples render as `Custom · 2x · 1.5GB`. No nearest-fit. *(Affects Phase 6 wizard Step 4 AND Phase 7 fleet row "Size" column.)*
4. **`MachineConfig.Metadata[fly_process_group]` = empty for M5.** No `MachineService` declared (decision 4); Fly's internal LB is not in the picture; the metadata key is omitted entirely. Revisit only if/when a future milestone adds public services.
5. **Pricing entirely cut from M5.** No JSON, no refresh cadence, no per-row cost column. Decision 15 in §2 is **superseded** — strike `frontend/src/lib/fly-pricing.ts` from Phase 6 / Phase 10; the spawn wizard's Step 5 (Review) renders the resolved Fly machine config without an estimated-cost line. The §1 item-6 spawn-wizard description's "+ estimated monthly cost (compute + storage)" is removed. Revisit only if operators ask for it post-M5.
6. **Skeleton/loading for the deployment inspector first `dry_run` round-trip = reuse the spawn wizard's `<DeployLog>` submitting pattern** (the M4 `<DeployModal>` referenced in the original plan was deleted in 0.9.2; `<DeployLog>` in `frontend/src/components/spawn/wizard.tsx` is its successor). Same skeleton register; consistent operator UX.
7. **Bulk apply's "Don't change" for `restart_max_retries` → disable + tooltip.** When policy = "Don't change" AND the selected agents have *mixed* policies, the `restart_max_retries` input is greyed out with tooltip *"only applies when policy is on-failure."* When all selected agents share the same policy (any value), the field stays editable. Explicit > implicit; no silent no-op writes.
8. **Drift summary granularity → bullet per drifted field (option A).** Inline diff per field on the row's drift banner, e.g. `Memory: 512MB → 2048MB · CPU: 1x → 2x · Region: iad → ord`. Drift is expected to be rare in v1.5; inline detail makes the "click to fix" path obvious without a click-to-expand round-trip. *(Affects Phase 7 banner UX.)*
9. **Mount path for non-Hermes harnesses → `agent_volumes.mount_path` column ships, populated from a constant in `internal/agents/service.go`.** M5 hardcodes `/opt/data` (Hermes' `$HERMES_HOME`) in the service layer when inserting `agent_volumes` rows. When v1.5+ adds a second harness, add `harness_adapters.default_mount_path` and rewire the service to read from there — zero schema change required, only a service-layer rewire. DB shape is forward-compatible from day one.
10. **`agent_volumes.fly_machine_id` stays nullable forever.** "Unattached volume" is a legitimate state to model — the orphan case (volume created, machine launch failed mid-Spawn) happens in the wild, and modeling it cleanly opens the rescue/reattach use case for free. Callers must handle the NULL when reading. **Operator UX consequence:** the per-agent inspector must surface unattached volumes and offer two actions — *(a) attach a fresh machine to the existing volume* (rescue), *(b) delete the unattached volume + respawn the agent* (clean restart). These two actions are M5-scope; Phase 7's inspector design must include them.
...
- [ ] Spawn wizard's Step 4 (Deployment) carries the live `DeployConfig` fields (incl. volume size); Step 5 (Review) renders the resolved Fly machine config; spawning with non-default region/size/volume/replicas works end-to-end. *(Storage cost line dropped per Q5.)*
